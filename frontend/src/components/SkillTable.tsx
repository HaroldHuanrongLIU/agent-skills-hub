              ...
              <td className="px-4 py-3">
                <div className="flex items-center gap-1">
                  ...
                  {isNew(skill.first_seen) && (
                    <span className="px-1 py-0.5 text-[9px] font-bold rounded bg-green-50 dark:bg-green-900/30 text-green-600">
                      NEW
                    </span>
                  )}
                  <OfficialBadge isOfficial={skill.is_official} />
                  <SecurityBadge grade={skill.security_grade} hideUnaudited />
                </div>
                  ...
                  <div className="w-16 h-1.5 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full ${
                        skill.quality_score >= 70
                          ? "bg-emerald-400"
                          : skill.quality_score >= 40
                            ? "bg-blue-400"
                            : "bg-gray-300"
                      }`}
                      style={{
                        width: `${Math.min(skill.quality_score, 100)}%`,
                      }}
                    />
                  </div>
                  <span className="text-xs text-gray-500 dark:text-gray-400 tabular-nums w-6">
                    {Math.round(skill.quality_score)}
                  </span>
                  <span className="text-[9px] font-bold text-gray-400 dark:text-gray-500">
                    {getTier(skill.quality_score)}
                  </span>
                </div>
              </td>
